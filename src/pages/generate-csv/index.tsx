...
const CsvGenerate = () => {
  ...
  const inputRefPhone = useRef<any>(null);
  const inputRefQtdLine = useRef<any>(null);
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      if (inputRefPhone?.current?.value) {
        data.append('targetPhoneNumber', inputRefPhone?.current?.value);
      }

      if (inputRefQtdLine?.current?.value) {
        data.append('qtdLine', inputRefQtdLine?.current?.value);
      }
      data.append('csvFile', selectedImage, selectedImage.name);
      const result = await axios.post(`${BASE_URL}/generateCSV`, data);
      if (result?.data?.data && result?.data?.data?.length > 0) {
        downloadCSVFromBase64(result?.data?.data, `${selectedImage.name.split('.')?.[0]}-atualizado.csv`);
      }
      setLoading(false);
  ...
  return (
    <>
      <Head>
        <title>Cnpj Biz Scrapping - Gerador de Csv</title>
      ...
      <div className="flex justify-center items-center">
        <input
          ref={inputRefPhone}
          type="text"
          className="last-removed-input"
          placeholder="Insira o último telefone enviado"
        />
      </div>
      <div className="flex justify-center items-center">
        <input
          ...
          type="text"
          className="last-removed-input"
          placeholder="Insira a quantidade de números enviados"
        />
      </div>
