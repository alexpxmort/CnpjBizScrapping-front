...
const Home = () => {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      data.append('data', selectedImage, selectedImage.name);
      const result = await axios.post(`${BASE_URL}/data-house`, data);
      if (result?.data?.data && result?.data?.data?.length > 0) {
        downloadCSVFromBase64(result?.data?.data, `${selectedImage.name.split('.')?.[0]}.csv`);
      }
      setLoading(false);
